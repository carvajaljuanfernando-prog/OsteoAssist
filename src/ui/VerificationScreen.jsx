...
/** Caso en blanco: mismas filas, sin valores. Para escribir un caso propio. */
const EMPTY_EXTRACTION = {
  patient: { name: null, documentId: null, birthDate: null, age: null, sex: "F", weightKg: null, heightCm: null },
  study: { date: null, manufacturer: null, model: null, facility: null, referenceDatabase: null },
  measurements: ["L1", "L2", "L3", "L4", "columna_lumbar", "cuello_femoral", "cadera_total", "radio_33"]
    .map((site) => ({ site, bmd: null, tScore: null, zScore: null, range: null, artifact: false, artifactNote: null })),
  tbs: null, vfa: null, priorStudy: null, reportImpression: null, lowConfidenceFields: [],
};
...
export default function VerificationScreen() {
  ...
  const touched =
    Object.keys(verifiedBlocks).length > 0 || imageUrl !== null || status !== "idle";

  /** Deja la pantalla lista para otro paciente. Nada se conserva entre casos. */
  const resetTo = (nextExtraction) => {
    if (touched && !window.confirm("Se perderán los datos del caso actual. ¿Continuar?")) return;
    ...
    <div className="min-h-screen bg-white text-slate-900" style={{ fontFamily: "ui-sans-serif, system-ui, sans-serif" }}>
      ...
      <header className="sticky top-0 z-10 bg-white border-b border-slate-300">
        <div className="max-w-6xl mx-auto px-5 py-3 flex flex-wrap items-center gap-4">
          ...
          <button
            onClick={() => resetTo(EMPTY_EXTRACTION)}
            className="text-sm px-3 py-2 rounded border border-slate-400 text-slate-700 hover:bg-slate-100"
          >
            Nuevo caso
          </button>
          <button
            onClick={() => resetTo(DEMO_EXTRACTION)}
            className="text-sm px-3 py-2 rounded text-slate-600 hover:bg-slate-100"
            title="Vuelve al caso de ejemplo con la artrosis de L3"
          >
            Ejemplo
          </button>
          <button
